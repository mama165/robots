...
func (s SecretManager) FindSecret(ctx context.Context, robots []Robot) {
	...
	supervisor := sp.NewSupervisor(ctx)

	// Running two goroutines for each robot to start
	for _, robot := range robots {
		supervisor.Add(workers.NewProcessSummaryWorker(s.Config, s.Log, &robot))
		supervisor.Add(workers.NewUpdateWorker(s.Config, s.Log, &robot))
		supervisor.Add(workers.NewSuperviseRobotWorker(s.Config, s.Log, &robot, winner))
		supervisor.Add(workers.NewStartGossipWorker(s.Config, s.Log, &robot, robots))
	}
	// Only the winner goroutine handle the writing
	supervisor.Add(workers.NewWriteSecretWorker(s.Config, s.Log, winner))
}
